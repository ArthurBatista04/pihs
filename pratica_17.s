...
.section .data
	...
	pulalin: .asciz "\n"
	formato1: .asciz "%f" # para simples precis�o
	formato2: .asciz "%lf" # para dupla precis�o
...
# 1) l� um n�mero ponto flutuante single (4 bytes) e o mostra como um n�mero double (8 bytes)
...
# Observacao 1: no trecho de c�digo anterior, foi lido um float de 4 bytes, mas o mesmo foi mostrado como um float de 8 bytes. Isso foi feito porque printf sempre considera 8 bytes para floats, seja formatado com %f ou %lf 

# Observacao 2: note que para mostrar o n�mero lido com scanf usamos a sequencia de instrucoes fldx+subl+fstx
	...
	call printf 		# ocorrer� um erro na impress�o
	addl $4, %esp
	...
	fldl float2 		# recoloca float2 no topo da pilha PFU
	fmul %st(1), %st(0) 	# faz %st(0) * %st(1) e sobrescreve
				# em %st(0)
	subl $8, %esp
	fstpl (%esp) 		# remove (pop) da Pilha PFU para a Pilha
				# do Sistema. O float1 fica no topo
